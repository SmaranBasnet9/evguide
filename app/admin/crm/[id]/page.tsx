    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-extrabold text-gray-900">{lead.displayId}</h1>
          <p className="mt-2 max-w-3xl text-gray-500">
            Journey stage: <strong className="font-semibold text-gray-700">{lead.journey_stage}</strong>.
            {" "}
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-500 shadow-sm">
          <p>Intent score: <strong className="text-gray-900">{lead.intent_score}</strong></p>
          <p className="mt-1">CRM status: <strong className="text-gray-900">{lead.crm_status}</strong></p>
          <p className="mt-1">Priority: <strong className="text-gray-900">{lead.crm_priority}</strong></p>
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-sm text-gray-500">Top interest</p>
          <p className="mt-2 text-xl font-bold text-gray-900">{lead.strongestCarLabel ?? "-"}</p>
          <p className="mt-1 text-xs text-gray-400">{lead.favorite_brand ?? "No favorite brand inferred"}</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-sm text-gray-500">Research depth</p>
          <p className="mt-2 text-xl font-bold text-gray-900">{lead.total_page_views} page views</p>
          <p className="mt-1 text-xs text-gray-400">{lead.primary_paths.join(" • ") || "No path pattern yet"}</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-sm text-gray-500">Buyer style</p>
          <p className="mt-2 text-xl font-bold text-gray-900">{lead.inferred_buyer_style ?? "Not enough signal"}</p>
          <p className="mt-1 text-xs text-gray-400">{lead.inferred_buyer_style_reason ?? "No stable inference yet."}</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
          <p className="text-sm text-gray-500">Last active</p>
          <p className="mt-2 text-xl font-bold text-gray-900">{formatDate(lead.last_activity_at)}</p>
          <p className="mt-1 text-xs text-gray-400">Follow-up: {formatDate(lead.next_follow_up_at)}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-6">
          <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {events.length === 0 ? (
                <p className="text-sm text-gray-500">No raw activity events found for this lead yet.</p>
              ) : (
                events.map((event) => (
                  <div key={event.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                    ...
                    {summarizeEventValue(event.event_value) && (
                      <p className="mt-2 text-xs text-gray-500">{summarizeEventValue(event.event_value)}</p>
                    )}
                  </div>
                ))
              )}
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {notes.length === 0 ? (
                ...
                notes.map((note) => (
                  <div key={note.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
                    <p className="text-sm text-gray-700">{note.body}</p>
                    <p className="mt-2 text-xs text-gray-400">{formatDate(note.created_at)}</p>
                  </div>
                ))
              )}
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-bold text-gray-900">Qualification hints</h2>
