    ...
    <div className="mx-auto max-w-7xl space-y-8">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-gray-900">Database and admin readiness</h1>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-5 py-4 text-sm text-gray-600 shadow-sm">
          <p>
            Generated: <span className="font-medium text-gray-900">{new Date(report.generatedAt).toLocaleString("en-GB")}</span>
          </p>
          <p className="mt-1">
            Admin client: <span className="font-medium text-gray-900">{report.adminClientReady ? "Ready" : "Not ready"}</span>
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold text-gray-900">{report.summary.healthy}</p>
        </div>
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
          <p className="text-sm font-medium text-amber-700">Warnings</p>
          <p className="mt-2 text-3xl font-bold text-amber-700">{report.summary.warning}</p>
        </div>
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 shadow-sm">
          <p className="text-sm font-medium text-rose-700">Missing or broken</p>
          <p className="mt-2 text-3xl font-bold text-rose-700">{report.summary.missing + report.summary.error}</p>
        </div>
        <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
          <p className="text-sm font-medium text-blue-700">Blocked admin routes</p>
          <p className="mt-2 text-3xl font-bold text-blue-700">{blockedRoutes}</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-gray-50 p-5 shadow-sm">
          <p className="text-sm font-medium text-gray-600">Degraded routes</p>
          <p className="mt-2 text-3xl font-bold text-gray-900">{degradedRoutes}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr,0.8fr]">
        <section className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-100 px-6 py-4">
            <h2 className="text-lg font-semibold text-gray-900">Environment and migrations</h2>
          ...
          <div className="space-y-4 px-6 py-5">
            ...
            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
              <p className="text-sm font-semibold text-gray-900">Local migration files</p>
              <div className="mt-3 space-y-3">
                {report.migrationFiles.map((migration) => (
                  <div key={migration.file} className="flex items-start justify-between gap-4 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm">
                    <div>
                      <p className="font-medium text-gray-900">{migration.file}</p>
                      <p className="text-gray-500">{migration.description}</p>
                    </div>
        ...
        <section className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-100 px-6 py-4">
            <h2 className="text-lg font-semibold text-gray-900">Admin route impact</h2>
            <p className="mt-1 text-sm text-gray-500">
          ...
          <div className="space-y-3 px-6 py-5">
            {report.routes.map((route) => (
              <div key={route.route} className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                ...
                <p className="mt-2 text-sm text-gray-600">{route.message}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-6 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Schema checks</h2>
          <p className="mt-1 text-sm text-gray-500">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50 text-left">
                <th className="px-6 py-3 font-semibold text-gray-500">Check</th>
                <th className="px-6 py-3 font-semibold text-gray-500">Table</th>
                <th className="px-6 py-3 font-semibold text-gray-500">Columns</th>
                <th className="px-6 py-3 font-semibold text-gray-500">Used by</th>
                <th className="px-6 py-3 font-semibold text-gray-500">Migration</th>
                <th className="px-6 py-3 font-semibold text-gray-500">Status</th>
              </tr>
            </thead>
            <tbody>
              {report.checks.map((check) => (
                <tr key={check.id} className="border-b border-gray-100 align-top last:border-b-0">
                  <td className="px-6 py-4">
                    <p className="font-medium text-gray-900">{check.label}</p>
                    <p className="mt-1 text-xs text-gray-500">{check.message}</p>
                  </td>
                  <td className="px-6 py-4 text-gray-700">
                    <code className="rounded bg-gray-100 px-2 py-1 text-xs">{check.table}</code>
                  </td>
                  <td className="px-6 py-4 text-gray-600">{check.columns.join(", ")}</td>
                  <td className="px-6 py-4 text-gray-600">{check.usedBy.join(", ")}</td>
                  <td className="px-6 py-4 text-gray-600">
                    {check.localMigration ? (
                      <div>
                        <p className="font-medium text-gray-900">{check.localMigration}</p>
                        <p className="text-xs text-gray-500">{check.hasLocalMigration ? "Present in repo" : "Referenced but not found locally"}</p>
                      </div>
                    ) : (
                      <span className="text-gray-400">No local migration tracked</span>
                    )}
                  </td>
                  <td className="px-6 py-4">
      ...
      <section className="rounded-2xl border border-blue-200 bg-blue-50 p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-blue-900">Recommended next actions</h2>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Link href="/admin/seo" className="rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-50">
            Review SEO admin
          </Link>
        </div>
