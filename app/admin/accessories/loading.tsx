  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex gap-6 border-b border-gray-100 bg-gray-50 px-6 py-3">
          {Array.from({ length: 5 }).map((_, i) => <Sk key={i} className="h-4 w-20" />)}
        </div>
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="flex gap-6 border-b border-gray-100 px-6 py-4">
            <Sk className="h-10 w-10 rounded-lg" />
            <Sk className="h-4 w-32" /><Sk className="h-4 w-20" /><Sk className="h-4 w-16" />
