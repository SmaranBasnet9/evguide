...
export default function ArticleContent({ blocks }: ArticleContentProps) {
  return (
    <section className="bg-[#F8FAF9] text-white">
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:px-8 lg:py-20">
              ...
              switch (block.type) {
                ...
                case "paragraph":
                  return (
                    <p
                      key={block.id}
                      className={block.lead ? "max-w-3xl text-xl leading-9 text-[#1A1A1A]" : "max-w-3xl text-lg leading-9 text-[#6B7280]"}
                    >
                      {block.content}
                    </p>
                  );
                case "list":
                  return (
                    <section key={block.id} className="rounded-[2rem] border border-[#E5E7EB] bg-[#F8FAF9] p-7">
                      {block.title ? <h3 className="text-xl font-semibold text-white">{block.title}</h3> : null}
                      <ul className="mt-4 space-y-3">
                        {block.items.map((item) => (
                          <li key={item} className="flex gap-3 text-base leading-8 text-[#6B7280]">
                ...
                case "insight":
                  return (
                    <section key={block.id} className="rounded-[2rem] border border-cyan-400/20 bg-cyan-400/[0.08] p-7">
                      ...
                      <p className="mt-4 max-w-3xl text-lg leading-8 text-[#1A1A1A]">{block.content}</p>
                    </section>
                  );
                case "evs":
                  return (
                    <section key={block.id} className="space-y-6">
                      <div>
                        ...
                        <p className="mt-3 max-w-3xl text-lg leading-8 text-[#6B7280]">{block.description}</p>
                      </div>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-28 rounded-[2rem] border border-[#E5E7EB] bg-[#F8FAF9] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.35)]">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#6B7280]">Move from content to action</p>
            ...
            <div className="mt-6 space-y-3">
              ...
              <Link href="/compare" className="block rounded-full border border-[#E5E7EB] bg-[#F8FAF9] px-5 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/10">
                Compare EVs
              </Link>
              <Link href="/finance" className="block rounded-full border border-[#E5E7EB] bg-[#F8FAF9] px-5 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/10">
                Check finance
              </Link>
              <Link href="/vehicles" className="block rounded-full border border-[#E5E7EB] bg-[#F8FAF9] px-5 py-3 text-center text-sm font-semibold text-white transition hover:bg-white/10">
                Explore vehicles
              </Link>
            </div>
          </div>
